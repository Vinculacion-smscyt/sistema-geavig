...
TAXONOMIA_VIOLENCIA = {
    "VIOLENCIA EN EL AMBITO FAMILIAR": {
        "VIOLENCIA FAMILIAR": [
            ...
            "VIOLENCIA ECONOMICA", "VIOLENCIA PATRIMONIAL", "VIOLENCIA MORAL"
        ]
    },
    ...
    "VIOLENCIA EN EL AMBITO LABORAL Y DOCENTE": {
        "VIOLENCIA LABORAL": [
            ...
            "DISCRIMINACION LABORAL", "AMENAZAS", "INTIMIDACION", "HUMILLACIONES", "EXPLOTACION LABORAL"
        ],
        "VIOLENCIA DOCENTE": [
            "HOSTIGAMIENTO SEXUAL", "ACOSO SEXUAL", "DISCRIMINACION", 
            "HUMILLACION", "ABUSO DE PODER", "ABUSO DE AUTORIDAD ESCOLAR"
        ]
    },
    "VIOLENCIA EN LA COMUNIDAD": {
        "ESPACIO PUBLICO": [
            "ACOSO SEXUAL EN ESPACIOS PUBLICOS", "DISCRIMINACION", "DENIGRACION", "MARGINACION", "EXCLUSION"
        ]
    },
    "VIOLENCIA INSTITUCIONAL": {
        "INSTITUCIONES PUBLICAS": [
            "DISCRIMINACION INSTITUCIONAL", "OMISION", "NEGLIGENCIA", 
            "DILACION", "OBSTACULIZACION DE DERECHOS", "NEGACION DE SERVICIOS", "REVICTIMIZACION"
        ]
    },
    "VIOLENCIA POLITICA CONTRA LAS MUJERES EN RAZON DE GENERO": {
        "AMBITO POLITICO Y ELECTORAL": [
            "OBSTACULIZACION DE DERECHOS POLITICOS", "OBSTACULIZACION DEL EJERCICIO DEL CARGO", 
            "AMENAZAS", "INTIMIDACION", "DISCRIMINACION", "DESCALIFICACION", "DIFAMACION"
        ]
    },
    "VIOLENCIA DIGITAL": {
        "TECNOLOGIAS DE LA INFORMACION Y COMUNICACION": [
            "DIFUSION DE CONTENIDO INTIMO SIN CONSENTIMIENTO", "PUBLICACION DE IMAGENES INTIMAS", 
            "ACOSO DIGITAL", "AMENAZAS DIGITALES", "DANO A LA INTIMIDAD", "DANO A LA DIGNIDAD"
        ]
    },
    "VIOLENCIA MEDIATICA": {
        "MEDIOS DE COMUNICACION": [
            "ESTEREOTIPOS SEXISTAS", "DISCRIMINACION DE GENERO", "COSIFICACION", 
            "HIPERSEXUALIZACION", "DISCURSO DE ODIO SEXISTA", "APOLOGIA DE LA VIOLENCIA"
        ]
    },
    "VIOLENCIA FEMINICIDA": {
        "VIOLENCIA EXTREMA DE GENERO": [
            "CONDUCTAS DE ODIO", "DISCRIMINACION", "VIOLENCIA EXTREMA", "MUERTE VIOLENTA", "FEMINICIDIO"
        ]
    },
    "VIOLENCIA VICARIA": {
        "VIOLENCIA A TRAVES DE HIJAS E HIJOS": [
            "SUSTRACCION DE HIJAS E HIJOS", "RETENCION DE HIJAS E HIJOS", 
            "AMENAZA DE CUSTODIA", "IMPEDIMENTO DE CONVIVENCIA", "MANIPULACION DE HIJAS E HIJOS"
        ]
    }
}
...
def popup_alerta_formato(campo_nombre: str):
    ...
    if st.button("Entendido y Continuar", use_container_width=True, key=f"btn_alerta_{campo_nombre}"):
        st.rerun()
...
# POPUP DE CONFIRMACIÓN Y GUARDADO DE REGISTRO
...
def popup_confirmacion_guardado():
    ...
    confirmado_check = st.checkbox("☑️ Confirmo que la información ha sido verificada y es correcta para su registro definitivo.", key="chk_confirmar_guardado_dialog")
    
    col_b1, col_b2 = st.columns(2)
    with col_b1:
        if st.button("✅ CONFIRMAR Y GUARDAR", disabled=not confirmado_check, use_container_width=True, key="btn_dialog_confirmar"):
            ...
            st.rerun()
    with col_b2:
        if st.button("❌ Cerrar / Revisar datos", use_container_width=True, key="btn_dialog_cerrar"):
            st.rerun()
...
with tab1:
    ...
    st.header("2. Ubicación de los Hechos (Datos Obligatorios)")
    ...
    with u2:
        sector = st.text_input("Sector Asignado (Obligatorio)", value=sector_calculado, disabled=True, key=f"txt_sec_{fid}")
    ...
    with a1:
        ...
        with fa2_2:
            if agr_empleado == "SI":
                ...
                agr_ocupacion = "NO TRABAJA"

    # Botón de Guardado con validación estricta de campos obligatorios
    if st.button("💾 GUARDAR REGISTRO GEAVIG", use_container_width=True, type="primary"):
        # Validar campos obligatorios
        faltan_datos = False
        if smz_seleccionada == "SELECCIONAR..." or not smz_seleccionada or not sector_calculado:
            st.error("❌ El campo Supermanzana / Sector es obligatorio.")
            faltan_datos = True
        if not latitud:
            st.error("❌ El campo Latitud es obligatorio.")
            faltan_datos = True
        if not longitud:
            st.error("❌ El campo Longitud es obligatorio.")
            faltan_datos = True

        if not faltan_datos:
            # Almacenar en temporal para la confirmación
            st.session_state["temp_registro"] = {
